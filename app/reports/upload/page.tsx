...
import { UploadCloud, FileText, X, ShieldCheck, ArrowRight, Loader2, FileUp } from "lucide-react";
...
export default function UploadReportPage() {
  ...
  const processUpload = async () => {
    ...
    try {
      ...
      setError("Failed to upload report. Please ensure the file is valid and try again.");
      setLoading(false);
    }
  };
                ...
                <div className="flex items-center gap-4 bg-slate-50 border border-slate-200 p-4 rounded-xl w-full max-w-sm mb-8">
                  ...
                  <div className="flex flex-col text-left overflow-hidden flex-1">
                    ...
                    <span className="text-sm text-slate-500">
                      {file.type.includes('pdf') ? 'PDF Document' : 'Image'} • {(file.size / 1024 / 1024).toFixed(2)} MB
                    </span>
                  </div>
                  <button 
